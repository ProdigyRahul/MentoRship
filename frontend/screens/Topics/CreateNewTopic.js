...
export default function CreateNewTopic({ navigation }) {
  const [characterCount, setCharacterCount] = useState(0);
  const { userId, setUserId, topicId, setTopicId } = useContext(UserType);
  ...
  const handleNext = async () => {
    if (!topicName || !sessionDescription || selectedGoals.length === 0) {
    ...
    try {
      ...
      await setTopicId(response.data.topicId);

      navigation.navigate("AddTopicMembers");
    } catch (error) {
